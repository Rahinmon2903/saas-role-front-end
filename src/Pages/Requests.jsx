...
const Requests = () => {
  ...
  const analytics = {
    ...
    overdue: requests.filter((r) => getSLAStatus(r.dueDate).label === "Overdue").length,
  };
  ...
  if (loading) {
    return <div className="flex-1 bg-neutral-950 p-8 text-neutral-400">Loading…</div>;
  }
  ...
  return (
    <main className="flex-1 bg-neutral-950 min-h-screen p-8 text-neutral-200 space-y-8">
      ...
      <div className="flex gap-3 bg-neutral-900 border border-neutral-800 rounded-xl px-3 py-2 text-sm">
        ...
        <select value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value)} className="bg-transparent">
      ...
      {role === "user" && (
        <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 max-w-xl">
          ...
          <form onSubmit={createRequest} className="space-y-3">
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title" className="w-full bg-neutral-950 border border-neutral-700 rounded-lg px-3 py-2 text-sm" />
            <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" className="w-full bg-neutral-950 border border-neutral-700 rounded-lg px-3 py-2 text-sm" />

            <div className="flex gap-3">
              <select value={priority} onChange={(e) => setPriority(e.target.value)} className="bg-neutral-950 border border-neutral-700 rounded-lg px-3 py-2 text-sm">
              ...
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="bg-neutral-950 border border-neutral-700 rounded-lg px-3 py-2 text-sm">
                <option value="hardware">Hardware</option>
                <option value="software">Software</option>
                <option value="network">Network</option>
                <option value="access">Access</option>
            ...
            <button disabled={submitting} className="bg-blue-600 hover:bg-blue-700 transition px-4 py-2 rounded-lg text-sm">
      ...
      {role === "admin" ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-neutral-900 border-b border-neutral-800 text-neutral-500">
                <tr>
                  ...
                  <th className="p-4 text-left font-medium">Assignment</th>
                  <th className="p-4 text-right font-medium">History</th>
                </tr>
              </thead>
              ...
              <tbody>
                {filteredRequests.map((r) => {
                  ...
                  return (
                    <tr key={r._id} className="border-b border-neutral-800 hover:bg-neutral-800/40 transition">
                      <td className="p-4">
                        <p className="text-white font-medium">{r.title}</p>
                        <p className="text-xs text-neutral-500 truncate max-w-xs">{r.description}</p>
                      </td>
                      ...
                      <td className="p-4 space-y-2">
                        ...
                        <div className="text-xs space-y-1">
                          <p className="text-neutral-500">Due {new Date(r.dueDate).toLocaleDateString()}</p>
                          <span className={`px-2 py-1 rounded ${sla.style}`}>{sla.label}</span>
                        </div>
                      </td>

                      <td className="p-4 text-xs text-neutral-300">{r.createdBy?.name}</td>
        ...
        <div className="space-y-4">
          {filteredRequests.map((r) => (
            <div key={r._id} className="bg-neutral-900 border border-neutral-800 rounded-xl p-5 space-y-3">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl w-full max-w-lg">
            <div className="flex justify-between px-5 py-3 border-b border-neutral-800">
              <h3 className="text-white text-sm font-semibold">Request History</h3>
            ...
            <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
              {historyRequest.history.map((h, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div className="pb-4">
                    <p className="text-sm font-semibold text-white">{h.action.replace("_", " ").toUpperCase()}</p>
                    <p className="text-xs text-neutral-400">By {h.by?.name} ({h.by?.role})</p>
                    <p className="text-xs text-neutral-500">{new Date(h.at).toLocaleString()}</p>
                    {h.remark && <p className="text-xs text-neutral-300 mt-1">{h.remark}</p>}
                  </div>
                </div>
              ))}
            </div>
...
const MetricCard = ({ title, value, danger }) => (
  <div className={`bg-neutral-900 border border-neutral-800 rounded-xl p-4 ${danger ? "border-red-500/30" : ""}`}>
    <p className="text-xs text-neutral-500">{title}</p>
    <p className={`text-lg font-semibold ${danger ? "text-red-400" : "text-white"}`}>
      {value}
    </p>
  </div>
);

    ...
    return (
      <main className="flex-1 bg-neutral-950 min-h-screen p-8 space-y-10 text-neutral-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Stat label="Total Requests" value={adminStats.totalRequests} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <InfoCard>
            Pending requests require assignment or approval. Processed requests
            include approved and rejected records.
          </InfoCard>
        </div>
      </main>
    );
  ...
  if (role === "manager" && managerStats) {
    return (
      <main className="flex-1 bg-neutral-950 min-h-screen p-8 space-y-8 text-neutral-200">
        ...
        <PrimaryCard label="Pending Approvals" value={managerStats.pending} />

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Stat label="Total Assigned" value={managerStats.total} />
          <Stat label="Pending" value={managerStats.pending} highlight />
          <Stat label="Completed" value={managerStats.completed} />
        </div>

        <Card title="Next Action">
          Review pending requests and approve or reject them to keep workflows
          moving.
          <div className="mt-4">
            <button
    ...
    return (
      <main className="flex-1 bg-neutral-950 min-h-screen p-8 space-y-8 text-neutral-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Stat label="Completed" value={userStats.completed} />
        </div>

        <InfoCard>
          You will be notified when a request is approved or rejected.
        </InfoCard>
      </main>
    );
  }
